...
//  Copyright © 2017 IcedOutIndustries. All rights reserved.
//
...
vector<vector<vector<string> > > Partition(vector<vector<string> > dataPartition, uint8_t attributePosition)
{
    set<string> attributeValues;
    vector<vector<vector<string> > > partitioned;
    ...
    for(auto it = attributeValues.begin(); it != attributeValues.end(); it++)
    {
        ...
        for (auto itt = dataPartition.begin(); itt != dataPartition.end(); itt++)
        {
            if((*itt).at(attributePosition) == *it)
                tempGroup.push_back(*itt);
        }
        partitioned.push_back(tempGroup);
    }
...
float infoA(vector<vector<string> > dataPartition, uint8_t attributePosition, uint8_t targetPosition)
{
    ...
    vector<vector<vector<string> > > partitioned;
    partitioned = Partition(dataPartition, attributePosition);
    
    float currConditionalInfo = 0;
    for(auto it = partitioned.begin(); it != partitioned.end(); it++)
    {
        
        currConditionalInfo +=  (float) (*it).size() / (float) dataPartition.size() * info(*it, targetPosition);
        
    }
    
    return currConditionalInfo;
}
